...
export default function Note({ active = false, note }: NoteProp) {
  return (
    <Link
      href={`/notes/${note.uid}`}
      className={`w-full fade-in-20 h-full rounded-md py-7.5 md:px-5.5 px-2 flex flex-col md:gap-3 gap-4 hover:bg-neutral-900/50 transition-colors border border-neutral-950 ${active ? "md:bg-neutral-900/50 md:border-neutral-900" : "bg-neutral-950"}`}
    >
